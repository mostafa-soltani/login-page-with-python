...
def open_win():
    ...
    def cocc():
        ...
        # خواندن تمام یوزرنیم‌ها از فایل
        try:
            with open('pass_user.csv', 'r', newline='') as f:
                reader = csv.reader(f)
                for row in reader:
                    if row and row[0] == usarname:
                        messagebox.showwarning('Warning', 'This username already exists.')
                        return
        except FileNotFoundError:
            # اگر فایل وجود نداشت، مشکلی نیست، ساخته می‌شود
            pass

        # اضافه کردن یوزرنیم و پسورد جدید
        with open('pass_user.csv', 'a+', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([usarname, password])
            user_pass[usarname] = password

        messagebox.showinfo('All right', 'Password saved', icon='info')
        save_pass.destroy()


    save_pass.bind('<Return>',lambda e: cocc())
...
root.bind('<Return>',lambda e:see_existense())
